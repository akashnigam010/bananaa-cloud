...
CREATE TABLE `Socyal`.`CHECKIN` (
  ...
  CONSTRAINT `CHECKIN_USER_FK`
    FOREIGN KEY (`USER_ID`)
    REFERENCES `Socyal`.`USER` (`ID`),
  CONSTRAINT `CHECKIN_FEEDBACK_FK`
...
CREATE TABLE `Socyal`.`REWARD_GIVEN` (
  `ID` INT NOT NULL AUTO_INCREMENT,
  `CHECKIN_ID` INT NOT NULL,
  `REWARD_ID` INT NOT NULL,
  `COUNT` INT NULL,
  `REWARD_DATETIME` TIMESTAMP NULL,
  PRIMARY KEY (`ID`),
  CONSTRAINT `REWARD_GIVEN_CHECKIN_ID_FK`
    FOREIGN KEY (`CHECKIN_ID`)
    REFERENCES `Socyal`.`CHECKIN` (`ID`),
  CONSTRAINT `REWARD_GIVEN_REWARD_ID_FK`
    FOREIGN KEY (`REWARD_ID`)
    REFERENCES `Socyal`.`REWARDS` (`ID`))
COLLATE='utf8_general_ci'
ENGINE=InnoDB
ROW_FORMAT=DEFAULT
AUTO_INCREMENT=1;

CREATE TABLE `Socyal`.`DISCOUNT_GIVEN` (
  `ID` INT NOT NULL AUTO_INCREMENT,
  `CHECKIN_ID` INT NOT NULL,
  `AMOUNT` DECIMAL(10,2) NULL,
  `DISCOUNT_DATETIME` TIMESTAMP NULL,
  PRIMARY KEY (`ID`),
  CONSTRAINT `DISCOUNT_GIVEN_CHECKIN_ID_FK`
    FOREIGN KEY (`CHECKIN_ID`)
    REFERENCES `Socyal`.`CHECKIN` (`ID`))
COLLATE='utf8_general_ci'
ENGINE=InnoDB
